...
-}
prover maxdeg maxtime rad midp ix prec form intvarids queue qlength inittime prevtime maxdepth computedboxes initvol truevol -- stateTV 
...
--          "\nDepth : " ++ show depth ++ "\n"
        prover maxdeg maxtime rad midp ix prec form intvarids boxes (qlength-1) inittime currtime maxdepth (computedboxes+1) initvol newtruevol  -- stateTV  
...
--        enqueueBox stateTV (1,0,0,0.7) box
        putStr $
          ...
          "\nDepth : " ++ show depth ++ "\n\n"
--        prover maxdeg form intvarids boxes -- stateTV
...
-}
        {-
        prover maxdeg maxtime rad midp ix prec form intvarids (boxes Q.|> (depth+1,boxL) Q.|> (depth+1,boxR)) (qlength+1) inittime currtime (max (depth+1) maxdepth) (computedboxes+1) initvol truevol -- stateTV
            breadth-first above depth-first below
        -}
        prover maxdeg maxtime rad midp ix prec form intvarids ((depth+1,boxL) Q.<| (depth+1,boxR) Q.<| boxes) (qlength+1) inittime currtime (max (depth+1) maxdepth) (computedboxes+1) initvol truevol -- stateTV
    where
